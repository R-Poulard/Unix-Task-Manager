int do_time_exit_code(char * pathtasks_file,int fd_reply){	// Donne a cassini l'ensemble des temps d'execution de la tache pathtasks_file avec leurs code de sortie
    ...
    int size_extime = sizeof(int64_t)+sizeof(uint16_t);//taille d'un couple

    int taille_max=10;//taille maximal de couple int64/uint16 qu'on peut écrire dans le buffer
    int taille_buff =10*(size_extime);	// Taille de base d'un couple temps d'execution/codes de sortie *10 pour limiter les realloc
    char *buffer = malloc(taille_buff); //MALLOC : buffer va contenir la liste des couples temps d'execution/codes de sortie
    assert(buffer!=NULL);
    int rd;


    uint32_t taille = 0;//compteur pour indiquer au pipe le nombre de time+exitcode envoyé (sert aussi à garder un compte de la taille accumulé dans le buffer)
    ...
    while(rd = read(fd_std, buffer+taille*size_extime, size_extime) >0){
       ...
       buffer = realloc(buffer, taille_buff +(10*size_extime));//REALLOC : Augmentation de la taille de buffer pour ajouter d'autres ouples temps d'execution/code
            taille_max+=10;
            taille_buff+= (10* size_extime); //On garde la taille de buffer à jour
       }
    }
    ...
    write_in_block(fd_reply,buff_total,sizeof(uint16_t)+ sizeof(uint32_t)+taille*size_extime);//WRITE : Ecriture de la reponse a cassini
...
int cmd_time_exit_code(int fd_request,int fd_reply, uint64_t taskid, char *pathtasks){
	DIR * dir;
